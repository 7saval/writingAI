...
export class IntialMigration1766824599910 implements MigrationInterface {
    name = 'IntialMigration1766824599910'

    public async up(queryRunner: QueryRunner): Promise<void> {
        await queryRunner.query(`ALTER TABLE \`users\` ADD \`resetCode\` varchar(255) NULL`);
        await queryRunner.query(`ALTER TABLE \`users\` ADD \`resetCodeExpires\` datetime NULL`);
    ...
    public async down(queryRunner: QueryRunner): Promise<void> {
        ...
        await queryRunner.query(`ALTER TABLE \`users\` DROP COLUMN \`resetCodeExpires\``);
