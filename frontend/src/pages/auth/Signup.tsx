...
const Signup = () => {
  ...
  const onSubmit = async (data: SignupProps) => {
    ...
    if (!emailCheckMutation.isSuccess) {
      await showAlert("이메일 중복 확인을 해주세요.", "이메일 중복 확인");
      return;
    }
    ...
    try {
      ...
      await showAlert("회원가입이 완료되었습니다.", "회원가입");
      navigate("/login");
    } catch (error: any) {
              ...
              <form onSubmit={handleSubmit(onSubmit)}>
                <div className="flex flex-col gap-6">
                  ...
                  {errors.email ? (
                    ...
                    <p className="text-sm text-destructive">
                      {emailCheckMutation.error?.response?.data?.message ||
                        "이미 사용 중인 이메일입니다."}
                    </p>
                  ) : emailCheckMutation.isSuccess ? (
                    <p className="text-sm text-green-600">
                      {emailCheckMutation.data?.message ||
                        "사용 가능한 이메일입니다."}
                    </p>
                  ) : null}
                  <div className="grid gap-2">
