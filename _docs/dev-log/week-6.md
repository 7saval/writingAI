...
#### 🎯 오늘의 목표
- [x] 로그아웃 시 헤더 UI 업데이트 문제 해결

#### ✅ 완료한 작업
- ✅ 로그아웃 시 헤더 UI 업데이트 문제 해결

#### 📝 작업 상세

#### 🔧 해결한 문제
- 문제: 로그아웃 시 헤더의 닉네임과 드롭다운 스타일이 즉시 업데이트되지 않고, 서버 응답이나 페이지 새로고침 후에만 반영되는 현상 발생.
- 원인: 로그아웃 시 서버 API 호출 후 React Query의 캐시 무효화(`invalidateQueries`)에만 의존하여 인증 상태를 갱신하고 있었음. 이로 인해 `authStore`(Zustand)의 로컬 상태가 즉시 초기화되지 않아 UI 반영이 지연되는 현상 발생.
- 해결: `Header.tsx`의 `handleLogout` 함수에서 `storeLogout()`을 호출하여 로컬 상태(Zustand)를 즉시 초기화하도록 수정. 이를 통해 React Query의 캐시 무효화가 완료되기 전에도 UI가 즉각적으로 '로그아웃' 상태로 변경되도록 개선.
