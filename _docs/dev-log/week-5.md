...
#### 🎯 오늘의 목표
- [x] 구글 OAuth를 추가 정보 더 받아서 회원가입하도록 구현

#### ✅ 완료한 작업
- ✅ 구글 로그인 시 추가 정보(닉네임) 입력 프로세스 구현

#### 📝 작업 상세
- **[백엔드] 구글 로그인 로직 고도화**
    - `googleLogin`: 신규 사용자의 경우 DB에 즉시 저장하지 않고, 소셜 정보를 포함한 임시 토큰(`signupToken`)을 발행하여 반환하도록 변경
    - `completeSocialSignup`: 발행된 `signupToken`과 사용자가 입력한 닉네임을 검증하여 최종 `User` 및 `SocialAccount` 생성 처리
- **[프론트엔드] 추가 정보 입력 플로우 구현**
    - `Login.tsx`: 구글 로그인 응답의 `isNewUser` 플래그에 따라 `/extra-info` 페이지로 리다이렉트 처리
    - `ExtraInfo.tsx` (신규): 닉네임 입력 폼 및 가입 완료 시그널 전송 로직 구현
    - `auth.api.ts` & `useAuthMutations.ts`: 소셜 가입 완료용 API 및 Mutation 추가/수정

#### 🚨 이슈/트러블슈팅/질문
- [질문] `signupToken` 유무로 신규 유저를 구분할 수 있는데, `isNewUser` 플래그를 별도로 내려주는 이유는?
    - **가독성/명시성**: 코드 내에서 의도를 명확히 파악 가능 (`if(isNewUser)`)
    - **역할 분리**: 상태(신규 가입 대상 여부)와 수단(가입용 인증 토큰)을 분리하여 설계
    - **확장성**: 나중에 신규 가입 외 다른 목적으로 토큰을 사용할 경우의 혼선 방지
    - **API 문서화**: 클라이언트가 어떤 상태인지 더 명확하게 전달 가능
...
#### 🎯 오늘의 목표
- [x] accessToken, refreshToken 나눠서 발급

#### ✅ 완료한 작업
- ✅ 이중 JWT(Access/Refresh) 인증 시스템 구현
- ✅ 자동 토큰 갱신(Silent Refresh) 기능 구현
- ✅ 인증 체크 시 발생하는 불필요한 401 에러 로그 제거

#### 📝 작업 상세
- **[백엔드] 이중 토큰 발급 및 검증 로직 구현**
    - `login`, `googleLogin`, `completeSocialSignup` 시 `accessToken`(15분)과 `refreshToken`(7일) 동시 발급
    - `refreshToken`은 `httpOnly`, `secure`, `sameSite: 'lax'` 설정으로 쿠키에 저장하여 보안 강화
    - `POST /api/auth/refresh` 엔드포인트 구현: 쿠키의 리프레시 토큰으로 액세스 토큰 갱신
    - `ensureAuth` 미들웨어를 수정하여 `Authorization: Bearer` 헤더의 액세스 토큰 검증
- **[프론트엔드] 자동 토큰 갱신(Axios Interceptor) 구현**
    - `accessToken`을 메모리(Zustand store)에 저장하여 보안성 향상
    - **요청 인터셉터**: 모든 API 요청 헤더에 자동으로 액세스 토큰 첨부
    - **응답 인터셉터**: 401(만료) 에러 발생 시 `/refresh` API를 호출하여 토큰을 갱신하고 원래 요청 재시도

[이슈1] **Access Token과 Refresh Token의 비밀키 분리**
- **질문**: 두 토큰의 Secret Key가 달라야 하는가?
- **답변**: 보안상 분리하는 것이 매우 중요함.
    1. **보안 범위 분리**: 하나의 키가 유출되어도 다른 토큰은 안전함.
    2. **용도 혼동 방지**: 액세스 토큰을 리프레시 토큰 자리에 넣는 등의 오용을 서명 단계에서 차단 가능.
    3. **유연한 로테이션**: 액세스 토큰 키만 주기적으로 변경하여 보안을 강화할 수 있음.

[이슈2] **토큰 갱신 시 무한 루프 발생**
- **문제**: 리프레시 토큰이 만료되었을 때 `/refresh` 요청 자체가 401을 반환하고, 인터셉터가 이를 다시 갱신하려 시도하면서 무한 루프 발생.
- **해결**: Axios 응답 인터셉터에서 요청 URL이 `/auth/refresh`인 경우는 401이 발생하더라도 재시도를 하지 않도록 예외 처리 추가.

[이슈3] **초기 진입 시 불필요한 401 에러 로그**
- **문제**: 비로그인 사용자가 접속할 때 `verify-user`와 `refresh` API가 401을 반환하여 브라우저 콘솔에 빨간색 에러 로그가 남음.
- **해결**: 인증 확인용 API들은 토큰이 없더라도 401 대신 **200 OK**와 함께 `authenticated: false` 플래그를 반환하도록 수정하여 "정상적인 비로그인 상태"로 처리.
